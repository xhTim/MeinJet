...
void DrawVn4eta(double etaCut){
    ...
    for(int i=0;i<9;i++){
        ...
        for(int j=0;j<2;j++){
            ...
            int YPlo=hSignal[i][j]->GetXaxis()->FindBin(etaCut);
            int YPhi=34;
    ...
    gVn[0][1]->GetYaxis()->SetTitle(Form("V^{*}_{n#Delta}{2,|#Delta#eta^{*}|>%.1f}", etaCut)); 
    ...
    cVn->SaveAs(Form("../Figuren/Vn/Vn_vs_Nch_eta%.1f.png", etaCut));

    TFile *fOut = new TFile(Form("../Results/Vn/Vn_vs_Nch_eta%.1f.root", etaCut), "recreate");
    gVn[0][0]->Write("V1");
    gVn[0][1]->Write("V2");
    gVn[0][2]->Write("V3");
    fOut->Close();
